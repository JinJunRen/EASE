...
class SMOTEBagging(BaseEstimator, ClassifierMixin):
    def __init__(self,
                 n_samples=100,
                 min_ratio=1.0,
                 with_replacement=True,
    ...
    def fit(self, X, y):
        ...
        for ibagging in range(self.n_estimators):
            b = min(0.1*((ibagging%10)+3), 1)
            train_maj = df_maj.sample(frac=b, replace=True)
            if n_min<=10:
                train_min = df_min.sample(frac=1.0, replace=True)
            else:
                train_min = df_min.sample(frac=b, replace=True)
#            train_maj = df_maj.sample(frac=0.5, replace=True)
#            
...
class UnderBagging(BaseEstimator, ClassifierMixin):
    def __init__(self,
                 n_samples=100,
                 min_ratio=1.0,
                 with_replacement=True,
